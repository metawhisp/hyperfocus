...
36. **Radar context tiers — every Mac gets a brain** (user: "приложение должно подходить для разных версий маков — Apple Intelligence не у всех есть"). DistractionJudge picks the best tier available at runtime, deployment target stays macOS 15, no cloud (the on-device privacy promise is a product pillar): (a) universal tier, ANY Mac incl. Intel — lexicalOnMission: mission tokens (≥4 chars, first-5-chars prefix matching absorbs RU/EN inflections) searched in the OCR text; ≥2 hits (or all tokens of a short mission) = on-mission, no nudge; 5 unit tests; (b) macOS 26+ with Apple Intelligence enabled — the on-device LLM judge (canon #35). Plus dwell everywhere: a keyword must survive TWO consecutive scans (~24 s) before any judging — a flash of "youtube" in one frame is not drifting. NLEmbedding rejected as a middle tier (no Russian, verified). Cloud judge deliberately NOT added; possible later only as explicit opt-in.
37. **Corner-case hardening of the detection stack** (user: "боюсь, мало корнер-кейсов продумаешь" — 30-agent adversarial hunt, every fix below verified-confirmed first). (a) CRITICAL capture resolution: SCDisplay reports POINTS, config wants PIXELS — points/2 gave ¼-Retina frames where 12 pt text = 6 px and OCR read nothing (radar near-silent) or garbage (judge misfired); now captured at true pixel scale per display (NSScreen backingScaleFactor). (b) CRITICAL Cyrillic: Vision .fast has NO ru support (verified by repro) — switched to .accurate with ["en-US","ru-RU"]. (c) ALL displays are captured (was displays.first only: radar blind to the second monitor AND mis-judging the first); Hyperfocus's own windows excluded (the HUD's mission line polluted the judge's view of "screen text"). (d) Keywords match on word boundaries ("for you" no longer fires on "for your convenience"; " x.com" dropped — it substring-matched dropbox.com/wix.com; alias keys were unreachable for it anyway). (e) Alias lists include their own literal keyword; overbroad aliases ("видео", "стрим") removed — semantic relevance is the judge's job. (f) LLM verdict parsed by FIRST WORD (substring "NO" matched NOTED/UNKNOWN); prompt marks screen text as untrusted data. (g) Radar sleeps during away (.pauseTimer) and resumes on recovery; useScreenAnalysis re-checked live; showNudge requires a running session (async judge completions could nudge after the end); dwell state never crosses sessions. (h) Attention cone vs multi-monitor false away-loop → Settings toggle "Strict attention" (hf.strictAttention, default on; off = presence-only). 69 tests green (verdict-parsing + word-boundary suites added).
38. **FLIGHT DECK status-bar menu** (user: "это тоже сделай секси дизайном"). The system NSMenu can't be styled, so the MenuBarExtra switched to .menuBarExtraStyle(.window): a dark FD device panel (deviceHi→device gradient, FDDotGrid, lime corner glow) with a lime-dot + matrix HYPERFOCUS header and styled rows (SF icon + label + ⌘N/chevron, lime hover highlight; Quit tinted red). Debug is an inline expandable section (no native submenus in .window style) — DEBUG builds only. Rows dismiss the panel via a captured hosting window (WindowGrabber → orderOut); toggles/expander don't. ⌘N works while the panel is open (button keyboardShortcut).
39. **Living idle orb** (user: playful liveliness on the red waiting orb + spin summon). Shared source of truth Orb/OrbLife.swift (OrbLifeVariant ×6: blink/spin/gaze/pulse/tilt/orbit — the last just ROLLS the ring sideways and back, no sphere spin per user; OrbLifeFrame pure compute; OrbEyes) rendered by BOTH the HF_ORB_LIFE_PREVIEW gallery and the production FocusOrbView. Orb/OrbLifeModel.swift schedules a RANDOM beat every 30–60 min while idle (isIdle closure) and plays the SPIN entrance once from the menu's "Show Focus Orb" (showOrbFromMenu; launch showOrb has no flourish). FocusOrbView un-pauses its CPU-saving TimelineView only for the ~2 s beat then re-pauses (AppState forwards orbLife.objectWillChange). Double-guarded against clowning mid-session: beats render only while isStatic (idle) AND AppState.interrupt() clears any beat when leaving idle. Slide capped ±8 pt so the glow never clips the 76 pt window; entrance guarded to idle. DEBUG menu: "Orb: random life beat" / "Orb: spin entrance" to judge live.
40. **The away moment, rebuilt** (user-approved from HF_AWAY_PREVIEW + HF_ALARM_PREVIEW galleries). (a) Exit trap: hovering the away card's "Exit Session" already proves presence → kicks recovery (onReturn); clicking it opens ARE YOU SURE? whose primary is RETURN TO HYPERFOCUS (recovery running underneath, camera keeps it honest); a quiet "Exit anyway" remains the honest way out. (b) Angry orb: warning → grows to 1.15 and jitters; away → grows GRADUALLY to ×2 over 7 s, shakes harder, wears angry eyes (gallery V2; brows slant with rage); colors keep the existing smooth MorphAnimator blend; reduce-motion disables all of it. The orb window resizes (76→114/182 pt) around the dot so the grown glow never clips. (c) Aura ramp: the red frame swells ×3.5 (alpha; width/blur capped ×2.2) over its first 7 s of away. (d) Quick-start ghost digits adapt contrast: pixels around the orb sampled via an 8×8 ScreenCaptureKit grab (own windows excluded) when Screen Recording is granted, else system appearance — black digits + white halo on light desktops, white + black on dark. (e) Alarm tones: AlarmSound {noise (default), siren 500↔750 Hz, triple beep, massive heartbeat (55+82 Hz, saturated)} — Settings "Alarm tone" picker, hf.alarmSound; user picked siren + beeps + beefed-up heartbeat from six candidates.
41. **Launch at login + live menu-bar timer + Statistics window (planned)**. (a) LaunchAtLoginService (SMAppService.mainApp) reflects hf.launchAtLogin into the real login-item registry; Settings toggle wired; reconciled at bootstrap. (b) MenuBarLabel: the menu-bar item shows a live MM:SS countdown while a session runs (isRunning/away/recovering), the orb glyph otherwise. (c) Statistics window MOCKUP (HF_STATS_PREVIEW) approved-pending: custom FD tab bar OVERVIEW | ACHIEVEMENTS; Overview = stat cards (total focus, current/longest streak, sessions, best hour, laser rate) + weekly bar chart; Achievements = badge grid to ~100 across 8 families (firsts/volume/streaks/quality/rhythm/marathon/comeback/day) with locked-dim + per-badge progress bars. Access will be menu bar → "Statistics…". Sparkle auto-update + macOS Focus integration planned separately.
42. **Statistics window LIVE + 100-achievement catalog** (canon #41 implemented). StatsService aggregates SessionStore history (completed sessions only) → total focus, session count, current/longest day-streak (Monday-week, streak stays alive if today is still empty but yesterday had one), best start-hour, laser rate (zero-drift share), this-week minutes per weekday; 9 unit tests. StatsView = the approved mockup on real data: OVERVIEW (stat cards + weekly bar chart) + ACHIEVEMENTS (AchievementCatalog — all 100 unique entries from specs/achievements-catalog.md, grouped into 10 families, unlocked ones from AchievementsStore light up by id, N/100 progress bar). Menu bar → "Statistics…". Next: expand AchievementsStore.evaluateCompletion to award all 100 (+ 5 new Session fields for nudge-survived/start-latency/sound+camera/away-count/wand) and per-badge progress.
